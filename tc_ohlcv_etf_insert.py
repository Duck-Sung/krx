...
if int_diff_days >= 0:
    ...
    cursor.execute("SELECT t.T_ID from TICKER t WHERE  t.market in ('ETF') and not exists (select * from TC_OHLCV tc where tc.T_ID=t.t_id and to_char(tc.tc_date,'YYYYMMDD') BETWEEN :1 and :2)",(last_day_str,yesterday_str))
    for row in cursor.fetchall():
        print("ticker_etf:",row[0])
